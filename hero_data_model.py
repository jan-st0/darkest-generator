...
class CombatSkill:
    ...
    def buffs_formated(self) -> tuple[tuple[str, float], ...]:
        return tuple((buff.stat, buff.max_val) for buff in self.buffs)
...
class Hero:
    ...
    def parse_base_dmg(self) -> tuple[int, ...]:
        ...
        if len(numbers) != 2:
            raise ValueError(
                f"Hero base damage has invalid format with {self.class_name=} {self.base_stats_lvl6.Base_DMG}"
            )
        return tuple(map(int, numbers))

    def get_max_crit(self, default: float = 0.0) -> float:
        try:
